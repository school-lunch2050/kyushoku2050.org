...
function loadBg (type: Omit<typeof types[number], '?' | '!'> | 'empty'): Promise<string> {
  return loadImage(`/img/lunch/${type}.webp`)
}
    ...
    .then((img) => {
      ...
      for (let x = 0; x < width; x += block) {
        for (let y = 0; y < height; y += block) {
          ctx.drawImage(img, x, y, block, block, x, y, block, block)
        }
      }
      return new Promise<string>((resolve, reject) => canvas.toBlob((blob: Blob | null) => {
        if (blob !== null) {
          resolve(URL.createObjectURL(blob))
        } else {
          reject(new Error('couldnt turn into blob'))
        }
      }, 'image/webp'))
    }).then((url) => {
